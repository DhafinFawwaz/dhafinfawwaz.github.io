...
<section id="contact">
    ...
    <ul className='grid grid-cols-2 gap-2 place-items-center 2xs:flex 2xs:flex-row flex-wrap justify-center'>
        {Object.keys(ContactList).map((key, index) => {
            return <li key={index} className='bg-night-800 flex flex-col rounded-xl shadow-rim-sm max-w-44 w-full h-full ease-out-back duration-150 hover:scale-105 active:scale-107 active:bg-indigo-700 cursor-pointer relative drop-shadow-lg active:z-40 active:ring-4 ring-indigo-800'>
